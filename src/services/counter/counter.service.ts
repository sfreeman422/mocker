...
export class CounterService {
  ...
  public createCounterMuzzleMessage(text: string) {
  ...
  public sendCounterMuzzledMessage(
    ...
    const counterMuzzle:
      | ICounterMuzzle
      | undefined = this.counterPersistenceService.getCounterMuzzle(userId);
    if (counterMuzzle) {
      ...
      if (counterMuzzle!.suppressionCount < MAX_SUPPRESSIONS) {
        ...
        this.webService.sendMessage(
          channel,
          `<@${userId}> says "${this.createCounterMuzzleMessage(text)}"`
        );
      }
    }
  }
